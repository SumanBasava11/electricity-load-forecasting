...
def train(args):
    ...
    model = LSTMForecast(input_size=X.shape[2], hidden_size=args.hidden, num_layers=args.layers, horizon=args.horizon).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    loss_fn = torch.nn.L1Loss()  # MAE

    mlflow.start_run()
    mlflow.log_params({"hidden": args.hidden, "layers": args.layers, "lr": args.lr})
    best_val = float('inf')
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1}/{args.epochs} - train_loss: {np.mean(train_losses):.4f} - val_loss: {mean_val:.4f}")
        mlflow.log_metric("val_loss", float(mean_val), step=epoch)
        if mean_val < best_val:
            best_val = mean_val
            os.makedirs(args.save_dir, exist_ok=True)
            best_path = os.path.join(args.save_dir, "best_lstm.pth")
